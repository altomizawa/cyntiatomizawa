  ...
  return (
    <>
      ...
      <div className={`bg-stone-800 fixed top-0 duration-500 ${isOpen ? 'right-0' : '-right-full'}  w-full h-full lg:hidden z-50`}>
        <div className='flex flex-col gap-12 items-left justify-center h-full pl-24'>
          <a onClick={() => setIsOpen(false)} href='#' className='text-7xl font-extralight uppercase tracking-widest text-white relative link duration-300 hover:text-black hover:pl-4'><span className={`duration-500 absolute top-0 left-0 h-full w-0 bg-white -z-10`}/>Home</a>
          <a onClick={() => setIsOpen(false)} href='#work' className='text-7xl font-extralight uppercase tracking-widest text-white relative link duration-300 hover:text-black hover:pl-4'><span className={`duration-500 absolute top-0 left-0 h-full w-0 bg-white -z-10`}/>WORK</a>
          <a onClick={() => setIsOpen(false)} href='#about' className='text-7xl font-extralight uppercase tracking-widest text-white relative link duration-300 hover:text-black hover:pl-4'><span className={`duration-500 absolute top-0 left-0 h-full w-0 bg-white -z-10`}/>ABOUT</a>
          <a onClick={() => setIsOpen(false)} href='#contact' className='text-7xl font-extralight uppercase tracking-widest text-white relative link duration-300 hover:text-black hover:pl-4'><span className={`duration-500 absolute top-0 left-0 h-full w-0 bg-white -z-10`}/>CONTACT</a>
        </div>
      </div>
    </>
  )
